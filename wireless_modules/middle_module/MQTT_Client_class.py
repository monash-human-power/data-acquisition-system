...
# Note: You can remove all the PRINT statements when everything is tested
...
class Client:
    def __init__(self, client_id, mqtt_broker):
        ...
        :param mqtt_broker: A string holding domain name or IP address of the broker to connect to, to send and receive
                            data.
        :param callback_func: The function to be called whenever a message is received.
        """
    ...
    def connect_and_subscribe(self, topics_to_subscribe, callback_func):
        ...
        :param topics_to_subscribe: An array of topic to subscribe to.
                                    Eah element must be a string or byte literal (preferred)
        ...
        # Check for any incoming messages already waiting
    ...
    def mqtt_pub(self, data, topic):
        ...
        try:
            ...
            print("OSError, check line 79 of main.py")
            # restart_and_reconnect()

    def check_for_message(self):
        """
        Checks whether an incoming message from the MQTT broker on the subscribed topics is pending. This check is done
        within the mqtt_pub and connect_and_subscribe method as well, but it gives a way to check manually for any
