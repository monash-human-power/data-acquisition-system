...
parser = argparse.ArgumentParser(
    description='DAS serial test script', add_help=True)
...
def run(args):
    ...
    if platform.system() != "Linux":
        raise Exception(
            "Serial test only works on Linux. If you are on Windows, perhaps try using WSL.")
    ...
    def send_data_func(data): return os.write(master, (data + "\n").encode())

    send_data_func("start")

    if args.file is None:
        send_fake_data(send_data_func, args.time,
                       args.rate, immitate_teensy=True)
    else:
        send_csv_data(send_data_func, args.file,
                      args.jump, immitate_teensy=True)
